...
void dvc_ScaLBL_D3Q19_AAeven_Greyscale(double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
	...
	for (int s=0; s<S; s++){
        ...
        GeoFun = 1.75 / sycl::sqrt(150.0 * porosity * porosity * porosity);
        ...
        Fx = -porosity * mu_eff / perm * ux -
             porosity * GeoFun / sycl::sqrt(perm) * u_mag * ux + porosity * Gx;
        Fy = -porosity * mu_eff / perm * uy -
             porosity * GeoFun / sycl::sqrt(perm) * u_mag * uy + porosity * Gy;
        Fz = -porosity * mu_eff / perm * uz -
             porosity * GeoFun / sycl::sqrt(perm) * u_mag * uz + porosity * Gz;
        if (porosity==1.0){
            Fx=Gx;
            Fy=Gy;
            Fz=Gz;
...
void dvc_ScaLBL_D3Q19_AAodd_Greyscale(int *neighborList, double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
                                                 double *Poros,double *Perm, double *Velocity, double *Pressure,
                                                 sycl::nd_item<3> item_ct1){
	int n;
	// conserved momemnts
...
void dvc_ScaLBL_D3Q19_AAeven_Greyscale_IMRT(double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
	...
	for (int s=0; s<S; s++){
	    //........Get 1-D index for this thread....................
            ...
            //Update the total force to include linear (Darcy) and nonlinear (Forchheimer) drags due to the porous medium
            Fx = Den * (-porosity * mu_eff / perm * ux -
                        porosity * GeoFun / sycl::sqrt(perm) * u_mag * ux +
                        porosity * Gx);
            Fy = Den * (-porosity * mu_eff / perm * uy -
                        porosity * GeoFun / sycl::sqrt(perm) * u_mag * uy +
                        porosity * Gy);
            Fz = Den * (-porosity * mu_eff / perm * uz -
...
void dvc_ScaLBL_D3Q19_AAodd_Greyscale_IMRT(int *neighborList, double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
                                                 double *Poros,double *Perm, double *Velocity,double Den, double *Pressure,
                                                 sycl::nd_item<3> item_ct1){
...
void dvc_ScaLBL_D3Q19_AAodd_Greyscale_MRT(int *neighborList, double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
...
void dvc_ScaLBL_D3Q19_AAeven_Greyscale_MRT(double *dist, int start, int finish, int Np, double rlx,  double rlx_eff, double Gx, double Gy, double Gz,
                                                  double *Poros,double *Perm, double *Velocity,double rho0, double *Pressure,
                                                  sycl::nd_item<3> item_ct1){
...
void dvc_ScaLBL_D3Q19_GreyIMRT_Init(double *dist, int Np, double Den,
                                    sycl::nd_item<3> item_ct1)
{
	int n;
	int S = Np/NBLOCKS/NTHREADS + 1;
...
extern "C" void ScaLBL_D3Q19_AAeven_Greyscale(double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_Greyscale(
                        dist, start, finish, Np, rlx, rlx_eff, Fx, Fy, Fz,
                        Poros, Perm, Velocity, Pressure, item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_AAodd_Greyscale(int *neighborList, double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_Greyscale(
                        neighborList, dist, start, finish, Np, rlx, rlx_eff, Fx,
                        Fy, Fz, Poros, Perm, Velocity, Pressure, item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_AAeven_Greyscale_IMRT(double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double Den,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_Greyscale_IMRT(
                        dist, start, finish, Np, rlx, rlx_eff, Fx, Fy, Fz,
                        Poros, Perm, Velocity, Den, Pressure, item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_AAodd_Greyscale_IMRT(int *neighborList, double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double Den,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_Greyscale_IMRT(
                        neighborList, dist, start, finish, Np, rlx, rlx_eff, Fx,
                        Fy, Fz, Poros, Perm, Velocity, Den, Pressure, item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_AAodd_Greyscale_MRT(int *neighborList, double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double rho0,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_Greyscale_MRT(
                        neighborList, dist, start, finish, Np, rlx, rlx_eff, Fx,
                        Fy, Fz, Poros, Perm, Velocity, rho0, Pressure,
                        item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_AAeven_Greyscale_MRT(double *dist, int start, int finish, int Np, double rlx, double rlx_eff, double Fx, double Fy, double Fz,double *Poros,double *Perm, double *Velocity,double rho0,double *Pressure){
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_Greyscale_MRT(
                        dist, start, finish, Np, rlx, rlx_eff, Fx, Fy, Fz,
                        Poros, Perm, Velocity, rho0, Pressure, item_ct1);
            });
...
extern "C" void ScaLBL_D3Q19_GreyIMRT_Init(double *dist, int Np, double Den){
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_GreyIMRT_Init(dist, Np, Den, item_ct1);
            });
        /*
