...
void dvc_ScaLBL_D3Q19_AAodd_StokesMRT(int *neighborList, double *dist, double *Velocity, double *ChargeDensity, double *ElectricField, double rlx_setA, double rlx_setB, double Gx, double Gy, double Gz, double rho0, double den_scale, double h, double time_conv,bool UseSlippingVelBC,int start, int finish, int Np,
...
void dvc_ScaLBL_D3Q19_AAeven_StokesMRT(double *dist, double *Velocity, double *ChargeDensity, double *ElectricField, double rlx_setA, double rlx_setB, double Gx, double Gy, double Gz,double rho0, double den_scale, double h, double time_conv, bool UseSlippingVelBC, int start, int finish, int Np,
                                       sycl::nd_item<3> item_ct1){
	...
	//cudaProfilerStart();
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_StokesMRT(
                        neighborList, dist, Velocity, ChargeDensity,
                        ElectricField, rlx_setA, rlx_setB, Gx, Gy, Gz, rho0,
                        den_scale, h, time_conv, UseSlippingVelBC, start,
                        finish, Np, item_ct1);
            });
        ...
        dpct::get_default_queue().parallel_for(
            ...
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_StokesMRT(
                        ...
                        rlx_setB, Gx, Gy, Gz, rho0, den_scale, h, time_conv,
                        UseSlippingVelBC, start, finish, Np, item_ct1);
            });

        /*
        DPCT1010:299: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        int err = 0;
