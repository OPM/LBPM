...
void dvc_ScaLBL_Color_Init(char *ID, double *Den, double *Phi, double das, double dbs, int Nx, int Ny, int Nz,
...
void dvc_ScaLBL_Color_InitDistancePacked(char *ID, double *Den, double *Phi, double *Distance,
	...
	for (int s=0; s<S; s++){
		//........Get 1-D index for this thread....................
			...
			if (i == 0 || j == 0 || k == 0 || i == Nx-1 || j == Ny-1 || k == Nz-1){
				...
				Den[2*n+1] = dbs;
				Phi[n] = (das-dbs)/(das+dbs);
                                d = sycl::fabs(Distance[n]);
                                Phi[n] =
                                    (2.f * (sycl::exp(-2.f * beta * (d + xp))) /
                                         (1.f +
                                          sycl::exp(-2.f * beta * (d + xp))) -
                                     1.f);
                        }
		}
	}
...
void dvc_ScaLBL_Color_InitDistance(char *ID, double *Den, double *Phi, double *Distance,
		double das, double dbs, double beta, double xp, int Nx, int Ny, int Nz,
		sycl::nd_item<3> item_ct1)
{
	int n,N;
...
void dvc_ScaLBL_Color_BC(int *list, int *Map, double *Phi, double *Den, double vA, double vB, int count, int Np,
                         sycl::nd_item<3> item_ct1)
{
	int idx,n;
...
void dvc_ScaLBL_Color_BC_z(int *list, int *Map, double *Phi, double *Den, double vA, double vB, int count, int Np,
                           sycl::nd_item<3> item_ct1)
{
	int idx,n,nm;
	// Fill the outlet with component b
...
void dvc_ScaLBL_D3Q19_ColorGradient(char *ID, double *phi, double *ColorGrad, int Nx, int Ny, int Nz,
                                    sycl::nd_item<3> item_ct1)
{
	int n,N,i,j,k,nn;
	// distributions
...
//*************************************************************************
void dvc_ColorCollide( char *ID, double *disteven, double *distodd, double *ColorGrad,
...
void dvc_ScaLBL_D3Q7_ColorCollideMass(char *ID, double *A_even, double *A_odd, double *B_even, double *B_odd, 
...
//*************************************************************************
void dvc_DensityStreamD3Q7(char *ID, double *Den, double *Copy, double *Phi, double *ColorGrad, double *Velocity,
		double beta, int Nx, int Ny, int Nz, bool pBC, sycl::nd_item<3> item_ct1)
{
	char id;
...
void dvc_ScaLBL_ComputePhaseField(char *ID, double *Phi, double *Den, int N,
                                  sycl::nd_item<3> item_ct1)
{
	int n;
	double Na,Nb;
...
void dvc_ScaLBL_SetSlice_z(double *Phi, double value, int Nx, int Ny, int Nz, int Slice,
                           sycl::nd_item<3> item_ct1)
{
        int n = Slice * Nx * Ny +
...
void dvc_ScaLBL_CopySlice_z(double *Phi, int Nx, int Ny, int Nz, int Source, int Dest,
                            sycl::nd_item<3> item_ct1){
	double value;
        int n = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
                item_ct1.get_local_id(2);
        if (n < Nx*Ny){
...
void dvc_ScaLBL_D3Q19_AAeven_Color(int *Map, double *dist, double *Aq, double *Bq, double *Den, double *Phi,
...
void dvc_ScaLBL_D3Q19_AAodd_Color(int *neighborList, int *Map, double *dist, double *Aq, double *Bq, double *Den,
...
void dvc_ScaLBL_D3Q19_AAodd_ColorMomentum(int *neighborList, double *dist, double *Den,
...
void dvc_ScaLBL_D3Q19_AAeven_ColorMomentum(double *dist, double *Den, double *Velocity,
...
void dvc_ScaLBL_D3Q19_AAeven_ColorMass(double *Aq, double *Bq, double *Den,
...
void dvc_ScaLBL_D3Q19_AAodd_ColorMass(int *neighborList, double *Aq, double *Bq, double *Den,
		double *Velocity, double *ColorGrad, double beta, int start, int finish, int Np,
		sycl::nd_item<3> item_ct1){
...
void dvc_ScaLBL_D3Q7_AAodd_PhaseField(int *neighborList, int *Map, double *Aq, double *Bq, 
...
void dvc_ScaLBL_D3Q7_AAeven_PhaseField(int *Map, double *Aq, double *Bq, double *Den, double *Phi, 
...
void dvc_ScaLBL_D3Q19_Gradient(int *Map, double *phi, double *ColorGrad, int start, int finish, int Np,
			int strideY, int strideZ, sycl::nd_item<3> item_ct1){
	int idx,ijk,nn;
	// distributions
...
void dvc_ScaLBL_PhaseField_Init(int *Map, double *Phi, double *Den, double *Aq, double *Bq, int start, int finish, int Np,
                                sycl::nd_item<3> item_ct1){
	int idx,n;
	double phi,nA,nB;
        ...
        */
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, GRID) *
                              ...
                              sycl::range<3>(1, 1, 512)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_SetSlice_z(Phi, value, Nx, Ny, Nz, Slice,
                                          item_ct1);
            });
}
...
extern "C" void ScaLBL_Color_Init(char *ID, double *Den, double *Phi, double das, double dbs, int Nx, int Ny, int Nz){
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_Color_Init(ID, Den, Phi, das, dbs, Nx, Ny, Nz,
                                          item_ct1);
            });
}
...
extern "C" void ScaLBL_Color_InitDistance(char *ID, double *Den, double *Phi, double *Distance,
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_Color_InitDistance(ID, Den, Phi, Distance, das,
                                                  dbs, beta, xp, Nx, Ny, Nz,
                                                  item_ct1);
            });
}
...
extern "C" void ScaLBL_D3Q19_ColorGradient(char *ID, double *phi, double *ColorGrad, int Nx, int Ny, int Nz){
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_ColorGradient(ID, phi, ColorGrad, Nx, Ny,
                                                   Nz, item_ct1);
            });
}
...
extern "C" void ColorCollide( char *ID, double *disteven, double *distodd, double *ColorGrad,
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ColorCollide(ID, disteven, distodd, ColorGrad, Velocity,
                                     Nx, Ny, Nz, rlx_setA, rlx_setB, alpha,
                                     beta, Fx, Fy, Fz, pBC, item_ct1);
            });
}
...
extern "C" void ScaLBL_D3Q19_ColorCollide( char *ID, double *disteven, double *distodd, double *phi, double *ColorGrad,
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_ColorCollide(ID, disteven, distodd, phi,
                                                  ColorGrad, Velocity, Nx, Ny,
                                                  Nz, rlx_setA, rlx_setB, alpha,
                                                  beta, Fx, Fy, Fz, item_ct1);
            });
}
...
extern "C" void DensityStreamD3Q7(char *ID, double *Den, double *Copy, double *Phi, double *ColorGrad, double *Velocity,
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_DensityStreamD3Q7(ID, Den, Copy, Phi, ColorGrad,
                                          Velocity, beta, Nx, Ny, Nz, pBC,
                                          item_ct1);
            });
}
...
extern "C" void ScaLBL_ComputePhaseField(char *ID, double *Phi, double *Den, int N){
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_ComputePhaseField(ID, Phi, Den, N, item_ct1);
            });
}
extern "C" void ScaLBL_D3Q7_ColorCollideMass(char *ID, double *A_even, double *A_odd, double *B_even, double *B_odd,
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q7_ColorCollideMass(
                        ID, A_even, A_odd, B_even, B_odd, Den, Phi, ColorGrad,
                        Velocity, beta, N, pBC, item_ct1);
            });
}
...
extern "C" void ScaLBL_D3Q19_AAeven_Color(int *Map, double *dist, double *Aq, double *Bq, double *Den, double *Phi,
        ...
        /*
        DPCT1007:259: Migration of cudaProfilerStart is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStart();
        /*
        DPCT1026:260: The call to cudaFuncSetCacheConfig was removed because
        ...
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_Color(
                        Map, dist, Aq, Bq, Den, Phi, Vel, rhoA, rhoB, tauA,
                        tauB, alpha, beta, Fx, Fy, Fz, strideY, strideZ, start,
                        finish, Np, item_ct1);
            });
        /*
        ...
        */
        ...
        /*
        DPCT1007:261: Migration of cudaProfilerStop is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line. 
        //cudaProfilerStop();
}
...
extern "C" void ScaLBL_D3Q19_AAodd_Color(int *d_neighborList, int *Map, double *dist, double *Aq, double *Bq, double *Den, 
        ...
        /*
        DPCT1007:264: Migration of cudaProfilerStart is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStart();
        /*
        DPCT1026:265: The call to cudaFuncSetCacheConfig was removed because
        SYCL currently does not support configuring shared memory on devices.
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_Color(
                        d_neighborList, Map, dist, Aq, Bq, Den, Phi, Vel, rhoA,
                        rhoB, tauA, tauB, alpha, beta, Fx, Fy, Fz, strideY,
        ...
        */
        ...
        /*
        DPCT1007:266: Migration of cudaProfilerStop is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStop();
}
...
extern "C" void ScaLBL_D3Q7_AAodd_PhaseField(int *NeighborList, int *Map, double *Aq, double *Bq, 
        ...
        /*
        DPCT1007:269: Migration of cudaProfilerStart is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStart();
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q7_AAodd_PhaseField(NeighborList, Map, Aq, Bq,
                                                     Den, Phi, start, finish,
                                                     Np, item_ct1);
            });

        /*
        DPCT1010:271: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        int err = 0;
        ...
        /*
        DPCT1007:270: Migration of cudaProfilerStop is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStop();
}
...
extern "C" void ScaLBL_D3Q7_AAeven_PhaseField(int *Map, double *Aq, double *Bq, double *Den, double *Phi, 
        ...
        /*
        DPCT1007:273: Migration of cudaProfilerStart is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStart();
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q7_AAeven_PhaseField(
                        Map, Aq, Bq, Den, Phi, start, finish, Np, item_ct1);
            });
        /*
        ...
        */
        ...
        /*
        DPCT1007:274: Migration of cudaProfilerStop is not supported.
        */
		// BUILD_FIX_AFTER_MIGRATION : Commented below line.
        //cudaProfilerStop();
}
	...
	int strideZ=Nx*Ny;
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_Gradient(Map, Phi, ColorGrad, start,
                                              finish, Np, strideY, strideZ,
                                              item_ct1);
            });
        /*
        DPCT1010:277: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        int err = 0;
...
extern "C" void ScaLBL_PhaseField_Init(int *Map, double *Phi, double *Den, double *Aq, double *Bq, int start, int finish, int Np){
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_PhaseField_Init(Map, Phi, Den, Aq, Bq, start,
                                               finish, Np, item_ct1);
            });
        /*
        DPCT1010:279: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        int err = 0;
...
extern "C" void ScaLBL_D3Q19_AAeven_ColorMomentum(double *dist, double *Den, double *Vel,
        ...
        /*
        DPCT1026:281: The call to cudaFuncSetCacheConfig was removed because
        SYCL currently does not support configuring shared memory on devices.
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_ColorMomentum(
                        dist, Den, Vel, ColorGrad, rhoA, rhoB, tauA, tauB,
                        alpha, beta, Fx, Fy, Fz, start, finish, Np, item_ct1);
            });
        /*
        DPCT1010:282: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced with 0. You need to rewrite this
        code.
        */
        int err = 0;
...
extern "C" void ScaLBL_D3Q19_AAodd_ColorMomentum(int *d_neighborList, double *dist, double *Den, double *Vel,
        ...
        /*
        DPCT1026:284: The call to cudaFuncSetCacheConfig was removed because
        SYCL currently does not support configuring shared memory on devices.
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_ColorMomentum(
                        d_neighborList, dist, Den, Vel, ColorGrad, rhoA, rhoB,
                        tauA, tauB, alpha, beta, Fx, Fy, Fz, start, finish, Np,
                        item_ct1);
            });
        /*
...
extern "C" void ScaLBL_D3Q19_AAeven_ColorMass(double *Aq, double *Bq, double *Den, double *Vel,
        ...
        /*
        DPCT1026:287: The call to cudaFuncSetCacheConfig was removed because
        SYCL currently does not support configuring shared memory on devices.
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAeven_ColorMass(Aq, Bq, Den, Vel,
                                                      ColorGrad, beta, start,
                                                      finish, Np, item_ct1);
            });
        /*
...
extern "C" void ScaLBL_D3Q19_AAodd_ColorMass(int *d_neighborList, double *Aq, double *Bq, double *Den, double *Vel,
        ...
        /*
        DPCT1026:290: The call to cudaFuncSetCacheConfig was removed because
        SYCL currently does not support configuring shared memory on devices.
        */
        ...
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, NBLOCKS) *
                              ...
                              sycl::range<3>(1, 1, NTHREADS)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_D3Q19_AAodd_ColorMass(
                        d_neighborList, Aq, Bq, Den, Vel, ColorGrad, beta,
    ...
    */
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, GRID) *
                              ...
                              sycl::range<3>(1, 1, 512)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_Color_BC_z(list, Map, Phi, Den, vA, vB, count,
                                          Np, item_ct1);
            });
        /*
        ...
        */
        dpct::get_default_queue().parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, GRID) *
                              ...
                              sycl::range<3>(1, 1, 512)),
            [=](sycl::nd_item<3> item_ct1) {
                    dvc_ScaLBL_CopySlice_z(Phi, Nx, Ny, Nz, Source, Dest,
                                           item_ct1);
            });
}
