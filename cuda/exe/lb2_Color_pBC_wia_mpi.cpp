...
extern "C" void dvc_InitDenColor(int nblocks, int nthreads, int S, char *ID,
                                 ...
                                 double dbs, int Nx, int Ny, int Nz);
//*************************************************************************
...
*/
    ...
    //......................................................................
    for (i = 0; i < N; i++)
        SignDist.data[i] -= 0.5; // Solid appears half a pixel bigger
    //......................................................................
...
*/	//......................................................................................
    ...
    dvc_PackDenD3Q7(faceGrid, packThreads, dvcRecvList_x, recvCount_x,
                    recvbuf_x, 2, Den, N);
    dvc_PackDenD3Q7(faceGrid, packThreads, dvcRecvList_y, recvCount_y,
                    recvbuf_y, 2, Den, N);
    dvc_PackDenD3Q7(faceGrid, packThreads, dvcRecvList_z, recvCount_z,
                    recvbuf_z, 2, Den, N);
    dvc_PackDenD3Q7(faceGrid, packThreads, dvcRecvList_X, recvCount_X,
                    recvbuf_X, 2, Den, N);
    dvc_PackDenD3Q7(faceGrid, packThreads, dvcRecvList_Y, recvCount_Y,
    ...
    //...........................................................................
    dvc_PackValues(faceGrid, packThreads, dvcSendList_x, sendCount_x, sendbuf_x,
                   Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_y, sendCount_y, sendbuf_y,
                   Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_z, sendCount_z, sendbuf_z,
                   Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_X, sendCount_X, sendbuf_X,
                   Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_Y, sendCount_Y, sendbuf_Y,
    ...
    dvc_PackValues(faceGrid, packThreads, dvcSendList_Z, sendCount_Z, sendbuf_Z,
                   Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_xy, sendCount_xy,
                   sendbuf_xy, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_xY, sendCount_xY,
                   sendbuf_xY, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_Xy, sendCount_Xy,
    ...
    dvc_PackValues(faceGrid, packThreads, dvcSendList_XY, sendCount_XY,
                   sendbuf_XY, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_xz, sendCount_xz,
                   sendbuf_xz, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_xZ, sendCount_xZ,
                   sendbuf_xZ, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_Xz, sendCount_Xz,
    ...
    dvc_PackValues(faceGrid, packThreads, dvcSendList_XZ, sendCount_XZ,
                   sendbuf_XZ, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_yz, sendCount_yz,
                   sendbuf_yz, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_yZ, sendCount_yZ,
                   sendbuf_yZ, Phi, N);
    dvc_PackValues(faceGrid, packThreads, dvcSendList_Yz, sendCount_Yz,
    ...
    /*		dvc_UnpackValues(faceGrid, packThreads, dvcSendList_x, sendCount_x,sendbuf_x, Phi, N);
	 ...
	 dvc_UnpackValues(faceGrid, packThreads, dvcSendList_Z, sendCount_Z,sendbuf_Z, Phi, N);
	 */
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_x, recvCount_x,
                     recvbuf_x, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_y, recvCount_y,
                     recvbuf_y, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_z, recvCount_z,
                     recvbuf_z, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_X, recvCount_X,
                     recvbuf_X, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_Y, recvCount_Y,
    ...
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_Z, recvCount_Z,
                     recvbuf_Z, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_xy, recvCount_xy,
                     recvbuf_xy, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_xY, recvCount_xY,
                     recvbuf_xY, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_Xy, recvCount_Xy,
    ...
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_XY, recvCount_XY,
                     recvbuf_XY, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_xz, recvCount_xz,
                     recvbuf_xz, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_xZ, recvCount_xZ,
                     recvbuf_xZ, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_Xz, recvCount_Xz,
    ...
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_XZ, recvCount_XZ,
                     recvbuf_XZ, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_yz, recvCount_yz,
                     recvbuf_yz, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_yZ, recvCount_yZ,
                     recvbuf_yZ, Phi, N);
    dvc_UnpackValues(faceGrid, packThreads, dvcRecvList_Yz, recvCount_Yz,
    ...
    if (rank == 0)
        printf("---------------------------------------------------------------"
               "----\n");
    if (rank == 0)
        printf("timestep dEs Vn Awn Ans Aws Lwns \n");
    if (rank == 0)
        printf("---------------------------------------------------------------"
               "----\n");
    ...
    while (timestep < timestepMax) {
        ...
        //...................................................................................
        dvc_PackDist(faceGrid, packThreads, 1, dvcSendList_x, 0, sendCount_x,
                     sendbuf_x, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 4, dvcSendList_x, sendCount_x,
                     sendCount_x, sendbuf_x, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 5, dvcSendList_x, 2 * sendCount_x,
                     sendCount_x, sendbuf_x, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 6, dvcSendList_x, 3 * sendCount_x,
                     sendCount_x, sendbuf_x, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 7, dvcSendList_x, 4 * sendCount_x,
                     sendCount_x, sendbuf_x, f_even, N);
        //...Packing for X face(faceGrid,packThreads,1,7,9,11,13)................................
        dvc_PackDist(faceGrid, packThreads, 0, dvcSendList_X, 0, sendCount_X,
                     sendbuf_X, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 3, dvcSendList_X, sendCount_X,
                     sendCount_X, sendbuf_X, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 4, dvcSendList_X, 2 * sendCount_X,
                     sendCount_X, sendbuf_X, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 5, dvcSendList_X, 3 * sendCount_X,
                     sendCount_X, sendbuf_X, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 6, dvcSendList_X, 4 * sendCount_X,
                     sendCount_X, sendbuf_X, f_odd, N);
        //...Packing for y face(faceGrid,packThreads,4,8,9,16,18).................................
        dvc_PackDist(faceGrid, packThreads, 2, dvcSendList_y, 0, sendCount_y,
                     sendbuf_y, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 4, dvcSendList_y, sendCount_y,
                     sendCount_y, sendbuf_y, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 4, dvcSendList_y, 2 * sendCount_y,
                     sendCount_y, sendbuf_y, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 8, dvcSendList_y, 3 * sendCount_y,
                     sendCount_y, sendbuf_y, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 9, dvcSendList_y, 4 * sendCount_y,
                     sendCount_y, sendbuf_y, f_even, N);
        //...Packing for Y face(faceGrid,packThreads,3,7,10,15,17).................................
        dvc_PackDist(faceGrid, packThreads, 1, dvcSendList_Y, 0, sendCount_Y,
                     sendbuf_Y, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 3, dvcSendList_Y, sendCount_Y,
                     sendCount_Y, sendbuf_Y, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 5, dvcSendList_Y, 2 * sendCount_Y,
                     sendCount_Y, sendbuf_Y, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 7, dvcSendList_Y, 3 * sendCount_Y,
                     sendCount_Y, sendbuf_Y, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 8, dvcSendList_Y, 4 * sendCount_Y,
                     sendCount_Y, sendbuf_Y, f_odd, N);
        //...Packing for z face(faceGrid,packThreads,6,12,13,16,17)................................
        dvc_PackDist(faceGrid, packThreads, 3, dvcSendList_z, 0, sendCount_z,
                     sendbuf_z, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 6, dvcSendList_z, sendCount_z,
                     sendCount_z, sendbuf_z, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 6, dvcSendList_z, 2 * sendCount_z,
                     sendCount_z, sendbuf_z, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 8, dvcSendList_z, 3 * sendCount_z,
                     sendCount_z, sendbuf_z, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 8, dvcSendList_z, 4 * sendCount_z,
                     sendCount_z, sendbuf_z, f_odd, N);
        //...Packing for Z face(faceGrid,packThreads,5,11,14,15,18)................................
        dvc_PackDist(faceGrid, packThreads, 2, dvcSendList_Z, 0, sendCount_Z,
                     sendbuf_Z, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 5, dvcSendList_Z, sendCount_Z,
                     sendCount_Z, sendbuf_Z, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 7, dvcSendList_Z, 2 * sendCount_Z,
                     sendCount_Z, sendbuf_Z, f_even, N);
        dvc_PackDist(faceGrid, packThreads, 7, dvcSendList_Z, 3 * sendCount_Z,
                     sendCount_Z, sendbuf_Z, f_odd, N);
        dvc_PackDist(faceGrid, packThreads, 9, dvcSendList_Z, 4 * sendCount_Z,
                     sendCount_Z, sendbuf_Z, f_even, N);
        //...Pack the xy edge (edgeGrid,packThreads,8)................................
        dvc_PackDist(edgeGrid, packThreads, 4, dvcSendList_xy, 0, sendCount_xy,
                     sendbuf_xy, f_even, N);
        //...Pack the Xy edge (edgeGrid,packThreads,9)................................
        dvc_PackDist(edgeGrid, packThreads, 4, dvcSendList_Xy, 0, sendCount_Xy,
                     sendbuf_Xy, f_odd, N);
        //...Pack the xY edge (edgeGrid,packThreads,10)................................
        dvc_PackDist(edgeGrid, packThreads, 5, dvcSendList_xY, 0, sendCount_xY,
                     sendbuf_xY, f_even, N);
        //...Pack the XY edge (edgeGrid,packThreads,7)................................
        dvc_PackDist(edgeGrid, packThreads, 3, dvcSendList_XY, 0, sendCount_XY,
                     sendbuf_XY, f_odd, N);
        //...Pack the xz edge (edgeGrid,packThreads,12)................................
        dvc_PackDist(edgeGrid, packThreads, 6, dvcSendList_xz, 0, sendCount_xz,
                     sendbuf_xz, f_even, N);
        //...Pack the xZ edge (edgeGrid,packThreads,14)................................
        dvc_PackDist(edgeGrid, packThreads, 7, dvcSendList_xZ, 0, sendCount_xZ,
                     sendbuf_xZ, f_even, N);
        //...Pack the Xz edge (edgeGrid,packThreads,13)................................
        dvc_PackDist(edgeGrid, packThreads, 6, dvcSendList_Xz, 0, sendCount_Xz,
                     sendbuf_Xz, f_odd, N);
        //...Pack the XZ edge (edgeGrid,packThreads,11)................................
        dvc_PackDist(edgeGrid, packThreads, 5, dvcSendList_XZ, 0, sendCount_XZ,
                     sendbuf_XZ, f_odd, N);
        //...Pack the xz edge (edgeGrid,packThreads,12)................................
        //...Pack the yz edge (edgeGrid,packThreads,16)................................
        dvc_PackDist(edgeGrid, packThreads, 8, dvcSendList_yz, 0, sendCount_yz,
                     sendbuf_yz, f_even, N);
        //...Pack the yZ edge (edgeGrid,packThreads,18)................................
        dvc_PackDist(edgeGrid, packThreads, 9, dvcSendList_yZ, 0, sendCount_yZ,
                     sendbuf_yZ, f_even, N);
        //...Pack the Yz edge (edgeGrid,packThreads,17)................................
        dvc_PackDist(edgeGrid, packThreads, 8, dvcSendList_Yz, 0, sendCount_Yz,
                     sendbuf_Yz, f_odd, N);
        //...Pack the YZ edge (edgeGrid,packThreads,15)................................
        dvc_PackDist(edgeGrid, packThreads, 7, dvcSendList_YZ, 0, sendCount_YZ,
                     sendbuf_YZ, f_odd, N);
        //...................................................................................
        ...
        dvc_UnpackDist(faceGrid, packThreads, 0, -1, 0, 0, dvcRecvList_X, 0,
                       recvCount_X, recvbuf_X, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 3, -1, -1, 0, dvcRecvList_X,
                       recvCount_X, recvCount_X, recvbuf_X, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 4, -1, 1, 0, dvcRecvList_X,
                       2 * recvCount_X, recvCount_X, recvbuf_X, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 5, -1, 0, -1, dvcRecvList_X,
                       3 * recvCount_X, recvCount_X, recvbuf_X, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 6, -1, 0, 1, dvcRecvList_X,
                       4 * recvCount_X, recvCount_X, recvbuf_X, f_odd, Nx, Ny,
                       Nz);
        //...................................................................................
        //...Map recieve list for the x face: q=1,7,9,11,13..................................
        dvc_UnpackDist(faceGrid, packThreads, 1, 1, 0, 0, dvcRecvList_x, 0,
                       recvCount_x, recvbuf_x, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 4, 1, 1, 0, dvcRecvList_x,
                       recvCount_x, recvCount_x, recvbuf_x, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 5, 1, -1, 0, dvcRecvList_x,
                       2 * recvCount_x, recvCount_x, recvbuf_x, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 6, 1, 0, 1, dvcRecvList_x,
                       3 * recvCount_x, recvCount_x, recvbuf_x, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 7, 1, 0, -1, dvcRecvList_x,
                       4 * recvCount_x, recvCount_x, recvbuf_x, f_even, Nx, Ny,
                       Nz);
        //...................................................................................
        //...Map recieve list for the y face: q=4,8,9,16,18 ...................................
        dvc_UnpackDist(faceGrid, packThreads, 1, 0, -1, 0, dvcRecvList_Y, 0,
                       recvCount_Y, recvbuf_Y, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 3, -1, -1, 0, dvcRecvList_Y,
                       recvCount_Y, recvCount_Y, recvbuf_Y, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 5, 1, -1, 0, dvcRecvList_Y,
                       2 * recvCount_Y, recvCount_Y, recvbuf_Y, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 7, 0, -1, -1, dvcRecvList_Y,
                       3 * recvCount_Y, recvCount_Y, recvbuf_Y, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 8, 0, -1, 1, dvcRecvList_Y,
                       4 * recvCount_Y, recvCount_Y, recvbuf_Y, f_odd, Nx, Ny,
                       Nz);
        //...................................................................................
        //...Map recieve list for the Y face: q=3,7,10,15,17 ..................................
        dvc_UnpackDist(faceGrid, packThreads, 2, 0, 1, 0, dvcRecvList_y, 0,
                       recvCount_y, recvbuf_y, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 4, 1, 1, 0, dvcRecvList_y,
                       recvCount_y, recvCount_y, recvbuf_y, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 4, -1, 1, 0, dvcRecvList_y,
                       2 * recvCount_y, recvCount_y, recvbuf_y, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 8, 0, 1, 1, dvcRecvList_y,
                       3 * recvCount_y, recvCount_y, recvbuf_y, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 9, 0, 1, -1, dvcRecvList_y,
                       4 * recvCount_y, recvCount_y, recvbuf_y, f_even, Nx, Ny,
                       Nz);
        //...................................................................................
        //...Map recieve list for the z face<<<faceGrid,packThreads,6,12,13,16,17)..............................................
        dvc_UnpackDist(faceGrid, packThreads, 2, 0, 0, -1, dvcRecvList_Z, 0,
                       recvCount_Z, recvbuf_Z, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 5, -1, 0, -1, dvcRecvList_Z,
                       recvCount_Z, recvCount_Z, recvbuf_Z, f_odd, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 7, 1, 0, -1, dvcRecvList_Z,
                       2 * recvCount_Z, recvCount_Z, recvbuf_Z, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 7, 0, -1, -1, dvcRecvList_Z,
                       3 * recvCount_Z, recvCount_Z, recvbuf_Z, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 9, 0, 1, -1, dvcRecvList_Z,
                       4 * recvCount_Z, recvCount_Z, recvbuf_Z, f_even, Nx, Ny,
                       Nz);
        //...Map recieve list for the Z face<<<faceGrid,packThreads,5,11,14,15,18)..............................................
        dvc_UnpackDist(faceGrid, packThreads, 3, 0, 0, 1, dvcRecvList_z, 0,
                       recvCount_z, recvbuf_z, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 6, 1, 0, 1, dvcRecvList_z,
                       recvCount_z, recvCount_z, recvbuf_z, f_even, Nx, Ny, Nz);
        dvc_UnpackDist(faceGrid, packThreads, 6, -1, 0, 1, dvcRecvList_z,
                       2 * recvCount_z, recvCount_z, recvbuf_z, f_odd, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 8, 0, 1, 1, dvcRecvList_z,
                       3 * recvCount_z, recvCount_z, recvbuf_z, f_even, Nx, Ny,
                       Nz);
        dvc_UnpackDist(faceGrid, packThreads, 8, 0, -1, 1, dvcRecvList_z,
                       4 * recvCount_z, recvCount_z, recvbuf_z, f_odd, Nx, Ny,
                       Nz);
        //..................................................................................
        //...Map recieve list for the xy edge <<<edgeGrid,packThreads,8)................................
        dvc_UnpackDist(edgeGrid, packThreads, 3, -1, -1, 0, dvcRecvList_XY, 0,
                       recvCount_XY, recvbuf_XY, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the Xy edge <<<edgeGrid,packThreads,9)................................
        dvc_UnpackDist(edgeGrid, packThreads, 5, 1, -1, 0, dvcRecvList_xY, 0,
                       recvCount_xY, recvbuf_xY, f_even, Nx, Ny, Nz);
        //...Map recieve list for the xY edge <<<edgeGrid,packThreads,10)................................
        dvc_UnpackDist(edgeGrid, packThreads, 4, -1, 1, 0, dvcRecvList_Xy, 0,
                       recvCount_Xy, recvbuf_Xy, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the XY edge <<<edgeGrid,packThreads,7)................................
        dvc_UnpackDist(edgeGrid, packThreads, 4, 1, 1, 0, dvcRecvList_xy, 0,
                       recvCount_xy, recvbuf_xy, f_even, Nx, Ny, Nz);
        //...Map recieve list for the xz edge <<<edgeGrid,packThreads,12)................................
        dvc_UnpackDist(edgeGrid, packThreads, 5, -1, 0, -1, dvcRecvList_XZ, 0,
                       recvCount_XZ, recvbuf_XZ, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the xZ edge <<<edgeGrid,packThreads,14)................................
        dvc_UnpackDist(edgeGrid, packThreads, 6, -1, 0, 1, dvcRecvList_Xz, 0,
                       recvCount_Xz, recvbuf_Xz, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the Xz edge <<<edgeGrid,packThreads,13)................................
        dvc_UnpackDist(edgeGrid, packThreads, 7, 1, 0, -1, dvcRecvList_xZ, 0,
                       recvCount_xZ, recvbuf_xZ, f_even, Nx, Ny, Nz);
        //...Map recieve list for the XZ edge <<<edgeGrid,packThreads,11)................................
        dvc_UnpackDist(edgeGrid, packThreads, 6, 1, 0, 1, dvcRecvList_xz, 0,
                       recvCount_xz, recvbuf_xz, f_even, Nx, Ny, Nz);
        //...Map recieve list for the yz edge <<<edgeGrid,packThreads,16)................................
        dvc_UnpackDist(edgeGrid, packThreads, 7, 0, -1, -1, dvcRecvList_YZ, 0,
                       recvCount_YZ, recvbuf_YZ, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the yZ edge <<<edgeGrid,packThreads,18)................................
        dvc_UnpackDist(edgeGrid, packThreads, 8, 0, -1, 1, dvcRecvList_Yz, 0,
                       recvCount_Yz, recvbuf_Yz, f_odd, Nx, Ny, Nz);
        //...Map recieve list for the Yz edge <<<edgeGrid,packThreads,17)................................
        dvc_UnpackDist(edgeGrid, packThreads, 9, 0, 1, -1, dvcRecvList_yZ, 0,
                       recvCount_yZ, recvbuf_yZ, f_even, Nx, Ny, Nz);
        ...
        //...................................................................................
        //...................................................................................
        dvc_UnpackDenD3Q7(faceGrid, packThreads, dvcSendList_x, sendCount_x,
                          sendbuf_x, 2, Den, N);
        dvc_UnpackDenD3Q7(faceGrid, packThreads, dvcSendList_y, sendCount_y,
                          sendbuf_y, 2, Den, N);
        dvc_UnpackDenD3Q7(faceGrid, packThreads, dvcSendList_z, sendCount_z,
                          sendbuf_z, 2, Den, N);
        dvc_UnpackDenD3Q7(faceGrid, packThreads, dvcSendList_X, sendCount_X,
                          sendbuf_X, 2, Den, N);
        dvc_UnpackDenD3Q7(faceGrid, packThreads, dvcSendList_Y, sendCount_Y,
        ...
        //...................................................................................
        dvc_PackValues(faceGrid, packThreads, dvcSendList_x, sendCount_x,
                       sendbuf_x, Phi, N);
        dvc_PackValues(faceGrid, packThreads, dvcSendList_y, sendCount_y,
                       sendbuf_y, Phi, N);
        dvc_PackValues(faceGrid, packThreads, dvcSendList_z, sendCount_z,
                       sendbuf_z, Phi, N);
        dvc_PackValues(faceGrid, packThreads, dvcSendList_X, sendCount_X,
                       sendbuf_X, Phi, N);
        dvc_PackValues(faceGrid, packThreads, dvcSendList_Y, sendCount_Y,
            ...
            for (c = 0; c < ncubes; c++) {
                ...
                for (p = 0; p < 8; p++) {
                    if (Phase(i + cube[p][0], j + cube[p][1], k + cube[p][2]) >
                            0 &&
                        SignDist(i + cube[p][0], j + cube[p][1],
                                 k + cube[p][2]) > 0) {
                        nwp_volume += 0.125;
                    }
                }
                ...
                if (Interface(SignDist, solid_isovalue, i, j, k) == 1) {
                    ...
                    SOL_SURF(SignDist, 0.0, Phase, fluid_isovalue, i, j, k, Nx,
...
*/
                    TRIM(local_sol_pts, n_local_sol_pts, fluid_isovalue,
                         local_sol_tris, n_local_sol_tris, ns_pts, n_ns_pts,
                         ns_tris, n_ns_tris, ws_pts, n_ws_pts, ws_tris,
                         n_ws_tris, values, local_nws_pts, n_local_nws_pts);
                    ...
                    if (n_local_nws_pts > 0) {
                        EDGE(Phase, fluid_isovalue, SignDist, i, j, k, Nx, Ny,
                             Nz, nw_pts, n_nw_pts, nw_tris, n_nw_tris,
                             local_nws_pts, n_local_nws_pts);
                    } else {
                        MC(Phase, fluid_isovalue, SignDist, i, j, k, nw_pts,
                ...
                else if (Fluid_Interface(Phase, SignDist, fluid_isovalue, i, j,
                                         k) == 1) {
                    MC(Phase, fluid_isovalue, SignDist, i, j, k, nw_pts,
                       n_nw_pts, nw_tris, n_nw_tris);
                }
                ...
                for (r = 0; r < n_nw_tris; r++) {
                    ...
                    s1 = sqrt((A.x - B.x) * (A.x - B.x) +
                              (A.y - B.y) * (A.y - B.y) +
                              (A.z - B.z) * (A.z - B.z));
                    s2 = sqrt((A.x - C.x) * (A.x - C.x) +
                              (A.y - C.y) * (A.y - C.y) +
                              (A.z - C.z) * (A.z - C.z));
                    s3 = sqrt((B.x - C.x) * (B.x - C.x) +
                              (B.y - C.y) * (B.y - C.y) +
                              (B.z - C.z) * (B.z - C.z));
                    s = 0.5 * (s1 + s2 + s3);
            ...
            //...........................................................................
            MPI_Barrier(comm);
            MPI_Allreduce(&nwp_volume, &nwp_volume_global, 1, MPI_DOUBLE,
                          MPI_SUM, comm);
...
*/
                printf("%i %f %f %f %f %f %f %f \n", timestep, dEs,
                       nwp_volume_global, awn_global, ans_global, aws_global,
                       As_global, lwns_global);
            }
        }
    }
    //************************************************************************/
